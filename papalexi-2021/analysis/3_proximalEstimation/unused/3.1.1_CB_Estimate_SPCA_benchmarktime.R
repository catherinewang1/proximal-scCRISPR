...
save_intermediateATEs = 'no' # 'yes'/'no' whether to save intermedate ATEs as they are estimated
...
which_estimators = list(lm_YA        = FALSE,
                        OCB_2SLS     = FALSE,
                        OCB_2SLS_pci2s=TRUE,
                        OCB_2SLSReg  = FALSE,
                        OCB_GMM      = FALSE,
...
source(sprintf('%s/constructDataList.R', util_dir)) # helper functions for data manipulation
...
# To perform 1000 Tests with lmYA       is 4.55 seconds --> ~ .00455 sec per test
# To perform  100 Tests with 2SLS pci2s is 57.25 mins --> ~ 34.35 sec per test
# To perform 2 Tests with 2SLS pci2s is 1.13 seconds --> ~ .565 min per test
...
# =================== Get ATEs (sequentially) ====================================
times = c()
for(r in whichROWS) {
  print(sprintf('idx: %d     [%s]', r, Sys.time()))
  t0 = Sys.time()
  get_ATE_est(AY_idx = ATEargs[r, 1])
  # get_ATE_est0(AY_idx=ATEargs[r, 1])
  t1 = Sys.time()
  times = c(times, as.numeric(t1 - t0))
}


hist(times)
range(times) # 33.18044 37.22835
summary(times)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 33.18   34.34   35.00   35.03   35.76   37.23 
sd(times) # 0.8857504
