...
library(ggplot2)
library(cowplot)
require(MASS) # for negative binomial fit
...
dir.create(sprintf('%s/spca/cbgenes/%s/glmYAU', save_dir, AYZW_setting_name), recursive = TRUE, showWarnings = FALSE)
...
format_AYU <- function(AY_idx) {
  ...
  # -------------------------------------------    
  Y = gene_odm[[get_importance_rank(AY_row$Y),     c(A_idx, NT_idx)]]
...
get_ATE_est_glmYAU <- function(AY_idx, covariates=NULL, fitNB=FALSE) {
  # AY_idx = 8755; covariates = c('n_umis', 'lane', 'bio_rep', 'phase', 'p_mito')
  
  # format df
  dfAYU = format_AYU(AY_idx) # head(dfAYU); dim(dfAYU)
  
  # perform poisson regression without covariates (naive)
  poisYA = glm('Y ~ A', data = dfAYU, family = 'poisson')
  poisYA_sum = summary(poisYA)
  res = data.frame(method      = 'poisYA',
                   method_type = 'poisson',
                   numNC       = NA,
                   basis       = NA,
                   ATE  = poisYA_sum$coefficients['ATRUE',   'Estimate'],
                   se   = poisYA_sum$coefficients['ATRUE', 'Std. Error'],
                   pval = poisYA_sum$coefficients['ATRUE',   'Pr(>|z|)'])
  
  
  # perform poisson regression with known covariates
  ...
  poisYAU = glm(myFormula, data = dfAYU, family = 'poisson')
  poisYAU_sum = summary(poisYAU)
  
  res = rbind(res, 
              data.frame(method      = 'poisYAU',
                         method_type = 'poisson',
                         numNC       = NA,
                         basis       = NA,
                         ATE  = poisYAU_sum$coefficients['ATRUE',   'Estimate'],
                         se   = poisYAU_sum$coefficients['ATRUE', 'Std. Error'],
                         pval = poisYAU_sum$coefficients['ATRUE',   'Pr(>|z|)']))
  
  # Negative Binomial Fits
  if(fitNB) {
    # perform linear regression without covariates (naive)
    nbYA = MASS::glm.nb('Y ~ A', data = dfAYU, link = log,
                        control = glm.control(maxit = 50))
    nbYA_sum = summary(nbYA)
    res = rbind(res, 
                data.frame(method      = 'nbYA',
                     method_type = 'negativebinomial',
                     numNC       = NA,
                     basis       = NA,
                     ATE  = nbYA_sum$coefficients['ATRUE',   'Estimate'],
                     se   = nbYA_sum$coefficients['ATRUE', 'Std. Error'],
                     pval = nbYA_sum$coefficients['ATRUE',   'Pr(>|z|)']))
    
    # perform poisson regression with known covariates
    nbYAU = MASS::glm.nb(myFormula, data = dfAYU, link = log,
                           control = glm.control(maxit = 50))
    nbYAU_sum = summary(nbYAU)
    
    res = rbind(res, 
                data.frame(method      = 'nbYAU',
                           method_type = 'negativebinomial',
                           numNC       = NA,
                           basis       = NA,
                           ATE  = nbYAU_sum$coefficients['ATRUE',   'Estimate'],
                           se   = nbYAU_sum$coefficients['ATRUE', 'Std. Error'],
                           pval = nbYAU_sum$coefficients['ATRUE',   'Pr(>|z|)']))
  }
...
get_ATE_est_glmYAU(AY_idx = 1000, covariates = NULL, fitNB = TRUE)


get_ATE_est_catcherror <- function(AY_idx, covariates=NULL, fitNB=FALSE) {
  res_df = tryCatch({get_ATE_est_glmYAU(AY_idx=AY_idx, covariates=covariates, fitNB=fitNB)},
                    error = function(cond) {
...
#                        variation1 = c(       'n_umis', 'lane', 'bio_rep', 'phase', 'p_mito'))
# 
...
ATE_par = future.apply::future_mapply(get_ATE_est_catcherror,
                                      MoreArgs = list(covariates=myCovariates, fitNB=myFitNB),
...
# write.csv(x = ATE,
#           file = sprintf('%s/spca/cbgenes/%s/glmYAU/ATE_glmYAU.csv', save_dir, AYZW_setting_name),
#           row.names = FALSE)

# change name for different covariates
write.csv(x = ATE,
          file = sprintf('%s/spca/cbgenes/%s/glmYAU/ATE_glmYAU_loglibsize.csv', save_dir, AYZW_setting_name),
          row.names = FALSE)
...
if(F) {
  ...
  nbYApvals  = df |> filter(type == myType & method == 'nbYA' ) |> pull(pval) |> sort()
  nbYAUpvals = df |> filter(type == myType & method == 'nbYAU') |> pull(pval) |> sort()
  nbYApvals  = df |> filter(type == myType & method == 'poisYA' ) |> pull(pval) |> sort()
  nbYAUpvals = df |> filter(type == myType & method == 'poisYAU') |> pull(pval) |> sort()
  
  
  
  plot_df = rbind(data.frame(EstMethod = 'nbYA',
                             pvals = nbYApvals,
                             theoretical = ppoints(length(nbYApvals))),
                  data.frame(EstMethod = 'nbYAU',
                             pvals = nbYAUpvals,
                             theoretical = ppoints(length(nbYAUpvals)))) |> 
    data.frame() |>
    mutate(EstMethod = factor(EstMethod, levels = c('nbYA', 'nbYAU'))) |>
  ...
  p1
  ggsave(plot = p1,
         filename = sprintf('%s/spca/cbgenes/%s/glmYAU/pval_qqplot.pdf', save_dir, AYZW_setting_name),
         height = 5, width = 5)
  ...
  p2
  ggsave(plot = p2,
         filename = sprintf('%s/spca/cbgenes/%s/glmYAU/pval_qqplot_trans.pdf', save_dir, AYZW_setting_name),
         height = 5, width = 5)
