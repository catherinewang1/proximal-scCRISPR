# ------------------------------------------------------------------------------------------------ #
#                        Construct Negative Controls using Sparse PCA Loadings
#     Performs Sparse PCA (package: PMA, itten et al 007). 
#     Saves the loadings to be used as Negative Controls (Exposures and Outcomes) in Prox Inf
# Requires: normalized values
# Outputs: saves loadings at 
...
suppressPackageStartupMessages(library(PMA))
...
valid_gene_names = gene_dev |> 
                   dplyr::filter((!is_transcription_factor) & !(gene_name %in% grnaTarget_names)) |> 
                   dplyr::arrange(importance_rank)
...
# =================== Perform Regular PCA ==========================================================
# ==================================================================================================
# subsample to speed up computation time (e.g. not all 21k cells)

dim(gene_norm)
# N_subsample = 5000
N_subsample = ncol(gene_norm)
set.seed(12345)
gene_norm_validgenes = gene_norm[valid_gene_names$importance_rank, ]

if(N_subsample == ncol(gene_norm)) {
  gene_norm_SPC = t(gene_norm_validgenes) # sample 5k out of 21k cells
} else {
  gene_norm_SPC = t(gene_norm_validgenes[, sample(1:ncol(gene_norm), N_subsample)]) # sample 5k out of 21k cells
}
...
saveRDS(NC_loadings, 
        sprintf('%s/pca/NCloadings.rds', save_dir)) 
...
if(F) {
  ...
  NC_avg = matrix(NA, 
                  nrow = ncol(gene_norm_validgenes), # number of cells 
